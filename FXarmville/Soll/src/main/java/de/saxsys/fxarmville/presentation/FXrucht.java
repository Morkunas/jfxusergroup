...
public class FXrucht extends Parent {
    ...
    private void starteWachstum() {
        ...
        final DoubleBinding standDerReifung =
                Bindings.min(frucht.aktuelleLebenszeitProperty().divide(frucht.lebenszeitProperty().divide(2)), 1.0);

        scaleXProperty().bind(standDerReifung);
        scaleYProperty().bind(standDerReifung);

        final DoubleBinding standDerFaulung =
                Bindings.max(
                        Bindings.subtract(2, frucht.aktuelleLebenszeitProperty().divide(frucht.lebenszeitProperty())
                                .multiply(2)), 0d);
